...
temp = time.time()
curr_folder_path = '../../results_english/'
if len(sys.argv) != 2:
...
INDEX_PARTS_CACHE_LIMIT, TITLES_CACHE_LIMIT, MINN, MAXX, ACCESSED_TOKENS_CACHE_LIMIT, N, FREQUENCY_THRESHOLD, QUERY_TOKENS_BOUND = 75, 90, 1, len(secondary_index), 120, 21384756, 155000, 40
...
allowed_chars, tag_set = set(chars), set("tibclr")
...
def encode_token(given_token):
    if not set(given_token).issubset(allowed_chars):
        return given_token
...
def pre_process_query(given_string):
    ...
    return [encode_token(ss.stemWord(t)) for t in stripped_tokens if (len(t) > 1) and (len(t) <= 45) and (set(t).issubset(allowed_chars)) and (t not in to_be_discarded)]
